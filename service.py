...
@is_authenticated
@commit
def todo_update(id: int,name: Optional[str] = None, description: Optional[str] = None):
    if name is not None and description is not None:
        updating_query = """UPDATE todo SET name=%s, description=%s WHERE id=%s"""
        cursor.execute(updating_query, (name, description, id))
        return Response('Successfully updated todo', status_code=200)
    elif name is not None:
        updating_query = """UPDATE todo SET name=%s WHERE id=%s"""
        cursor.execute(updating_query, (name, id))
        return Response('Successfully updated todo', status_code=200)
    elif description is not None:
        updating_query = """UPDATE todo SET description=%s WHERE id=%s"""
        cursor.execute(updating_query, (description, id))
        return Response('Successfully updated todo', status_code=200)
    else:
        return Response('No updates provided', status_code=400)
    
@is_authenticated
@commit    
def todo_delete(id:int):
    deleting_query="""DELETE FROM todo WHERE id=%s; """
    cursor.execute(deleting_query,(id,))
    return Response('Sucessfully deleted todo', status_code=200)
